...
#ifdef __cplusplus
// @class	PosControl
// @brief	Position control class
class PosControl
{
public:

    /// Constructor
    PosControl(Motors &motors, Attitude_Multi& attitude_multi);
    ...
    void set_alt_target_to_current_alt() { _pos_target.z = get_pos_z(); }
    ...
    const float& get_alt_target() { return _pos_target.z; }
    ...
    const Vector3f& get_desired_accel() { return _accel_desired; }
    ...
    void update_xy_controller(float dt, float x_current, float y_current, float vx_current, float vy_current);
    ...
    /// get pid controllers
    P& get_pos_z_p() { return _p_pos_z; }
    P& get_vel_z_p() { return _p_vel_z; }
    PID& get_accel_z_pid() { return _pid_accel_z; }
    P& get_pos_xy_p() { return _p_pos_xy; }
    PID_2D& get_vel_xy_pid() { return _pid_vel_xy; }
...
protected:
    ...
    //          init_takeoff
    void run_z_controller(float alt_current, float vel_current);
    ...
    void run_xy_controller(float dt, float x_current, float y_current, float vx_current, float vy_current);
    ...
    /// initialise ekf position resets
    void init_ekf_xy_reset(){ekf_xy_reset();}
    void init_ekf_z_reset(){ekf_z_reset();}

    // references to inertial nav and ahrs libraries
    Attitude_Multi&    _attitude_multi;
    Motors& _motors;
